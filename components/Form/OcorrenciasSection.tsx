...
export default function OcorrenciasSection({ formData, onChange, gerenciarArray }: Props) {
  ...
  // --- FUNÇÕES DE INTERCEPTAÇÃO PARA AUDITORIA E TOASTS --- //

  const handleToggleCheckbox = (e: React.ChangeEvent<HTMLInputElement>, label: string) => {
    onChange(e); // Chama a função original para atualizar o estado
    ...
    if (e.target.checked) {
      toast.info(`Preencha os dados de ${label}.`);
    }
  };

  const handleAddArrayItem = (campo: keyof RelatorioData, label: string, novoItem: any) => {
  ...
  const handleRemoveArrayItem = (campo: keyof RelatorioData, index: number, label: string) => {
    ...
    <div className="space-y-6 mt-8">
      ...
      <section className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm transition-all hover:shadow-md">
        <label className="flex items-center gap-3 cursor-pointer group w-fit">
          <input 
            type="checkbox" 
            name="temSaida" 
            checked={formData.temSaida} 
            onChange={(e) => handleToggleCheckbox(e, 'Saída Externa')} 
      ...
      <section className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm transition-all hover:shadow-md">
        <label className="flex items-center gap-3 cursor-pointer group w-fit">
          <input 
            type="checkbox" 
            name="temAdmissao" 
            checked={formData.temAdmissao} 
            onChange={(e) => handleToggleCheckbox(e, 'Admissões')} 
      ...
      <section className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm transition-all hover:shadow-md">
        <label className="flex items-center gap-3 cursor-pointer group w-fit">
          <input 
            type="checkbox" 
            name="temDesligamento" 
            checked={formData.temDesligamento} 
            onChange={(e) => handleToggleCheckbox(e, 'Desligamentos')} 
      ...
      <section className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-6 transition-all hover:shadow-md">
        <div className="space-y-3">
          <label className="flex items-center gap-2 cursor-pointer group w-fit">
            <input 
              type="checkbox" 
              name="temFolga" 
              checked={formData.temFolga} 
              onChange={(e) => handleToggleCheckbox(e, 'Folgas')} 
        ...
        <div className="space-y-3">
          <label className="flex items-center gap-2 cursor-pointer group w-fit">
            <input 
              type="checkbox" 
              name="temFerias" 
              checked={formData.temFerias} 
              onChange={(e) => handleToggleCheckbox(e, 'Férias/Atestado')} 
        ...
        <div className="space-y-3">
          <label className="flex items-center gap-2 cursor-pointer group w-fit">
            <input 
              type="checkbox" 
              name="temApoioSemiliberdade" 
              checked={formData.temApoioSemiliberdade} 
              onChange={(e) => handleToggleCheckbox(e, 'Apoio Semiliberdade')} 
